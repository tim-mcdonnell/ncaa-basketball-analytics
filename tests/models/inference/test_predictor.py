...
from src.models.inference.predictor import (
    GamePredictor,
    batch_predict,
...
class TestFeatureProcessing:
    ...
    def test_create_feature_vector(self):
        ...
        assert feature_vectors.shape == (3, 3), f"Expected shape (3, 3), got {feature_vectors.shape}"
...
class TestBatchPrediction:
    ...
    def test_batch_predict(self, mock_model, sample_data):
        ...
        expected_preds = torch.sum(torch.tensor(expected_values, dtype=torch.float32), dim=1, keepdim=True) * 0.1
...
class TestGamePredictor:
    ...
    def test_predictor_initialization(self, mock_model):
        ...
        assert predictor.feature_columns == mock_model.config.features, \
    ...
    def test_predict_game(self, mock_model):
        ...
        assert np.isclose(prediction, expected_prediction), \
    ...
    def test_format_predictions(self, mock_model, sample_upcoming_games):
        ...
        formatted = predictor.format_predictions(
            games_df=sample_upcoming_games,
