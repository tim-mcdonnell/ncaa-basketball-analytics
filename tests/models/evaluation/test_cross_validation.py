...
class TestTimeSeriesSplit:
    ...
    def test_time_series_split(self):
        ...
        for i, (train_idx, test_idx) in enumerate(splits):
            ...
            assert set(train_data["id"].to_list()).isdisjoint(set(test_data["id"].to_list())), \
            ...
            if i < 4:  # All but the last fold
                ...
                assert abs(len(test_idx) - expected_test_size) <= 1, \
...
class TestKFoldCrossValidator:
    ...
    def test_kfold_validation(self, mock_model_factory, sample_data):
        ...
        with patch('src.models.training.trainer.ModelTrainer') as MockTrainer:
            ...
            assert isinstance(results, CrossValidationResults), "Should return CrossValidationResults"
            
            # Verify fold metrics
            assert len(results.fold_metrics) == 5, f"Expected metrics for 5 folds, got {len(results.fold_metrics)}"
            ...
            assert "mean_accuracy" in results.aggregate_metrics, "Aggregate metrics should include mean accuracy"
            assert "std_accuracy" in results.aggregate_metrics, "Aggregate metrics should include std accuracy"
...
class TestCrossValidationResults:
    ...
    def test_results_aggregation(self):
        ...
        # Verify accuracy
        assert np.isclose(agg_metrics["mean_accuracy"], expected_mean_accuracy), \
            f"Expected mean accuracy {expected_mean_accuracy}, got {agg_metrics['mean_accuracy']}"
        assert np.isclose(agg_metrics["mean_rmse"], expected_mean_rmse), \
