...
class TestGameDataset:
    ...
    def test_dataset_creation(self):
        ...
        assert features.shape == (len(feature_cols),), f"Expected shape ({len(feature_cols)},), got {features.shape}"
    ...
    def test_dataset_dataloader(self):
        ...
        # Verify batch shapes
        assert features_batch.shape == (batch_size, len(feature_cols)), \
            f"Expected features shape ({batch_size}, {len(feature_cols)}), got {features_batch.shape}"
        assert targets_batch.shape == (batch_size,), \
...
class TestDataSplitting:
    ...
    def test_train_val_test_split(self):
        ...
        data = pl.DataFrame({
            ...
            "game_date": [i for i in range(1, 101)],  # Use integers for dates to simplify testing
