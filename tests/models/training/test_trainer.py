...
class TestModelTrainer:
    ...
    def test_trainer_initialization(self, simple_model, sample_data):
        ...
        config = ModelConfig(
            ...
            hyperparameters={"learning_rate": 0.001, "weight_decay": 1e-5, "batch_size": 16},
        ...
        trainer = ModelTrainer(
            model=simple_model, train_loader=train_loader, val_loader=val_loader, config=config
        )
    ...
    def test_early_stopping(self, simple_model, sample_data, tmp_path):
        ...
        config = ModelConfig(
            model_type="test_model",
            hyperparameters={"learning_rate": 0.01, "weight_decay": 0, "batch_size": 16},
            features=["feature1", "feature2", "feature3"],
            training_params={
        ...
        try:
            ...
            assert (
                len(train_losses) <= 6
            ), f"Early stopping failed, trained for {len(train_losses)} epochs"
            assert (
                len(val_losses) <= 6
            ), f"Early stopping failed, validated for {len(val_losses)} epochs"
