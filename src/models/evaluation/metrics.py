...
def calculate_point_spread_accuracy(
    ...
    winner_accuracy = np.mean(correct_winner)
    ...
    for i in range(len(y_true)):
        details.append({
            "correct_winner": correct_winner[i],
            "prediction_error": abs(y_true[i] - y_pred[i]),
...
def calculate_calibration_metrics(
    ...
    try:
        ...
        reliability = np.sum(bin_weights := np.bincount(np.digitize(y_pred_norm, np.linspace(0, 1, n_bins+1))-1, minlength=n_bins) / len(y_pred_norm) * (prob_pred - prob_true)**2)
