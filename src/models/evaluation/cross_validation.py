...
class TimeSeriesSplit:
    ...
    def __init__(
        self, n_splits: int = 5, date_column: str = "game_date", test_size: Optional[int] = None
    ):
        """
...
class KFoldCrossValidator:
    ...
    def _create_splits(self) -> List[Tuple[np.ndarray, np.ndarray]]:
        ...
        if self.cv_type == "kfold":
            ...
            splitter = TimeSeriesSplit(
                ...
                test_size=len(self.data) // self.n_splits,
            )
    ...
    def run_cv(self) -> CrossValidationResults:
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(splits):
            ...
            train_data = data_with_index.filter(pl.col("_row_idx").is_in(train_idx)).drop(
                "_row_idx"
            )
            ...
            if hasattr(model, "_extract_mock_name") and callable(
                getattr(model, "_extract_mock_name", None)
            ):
                try:
                    # Create trainer
...
def create_kfold_splits(
    data: pl.DataFrame, n_splits: int = 5, shuffle: bool = True, random_state: int = 42
) -> List[Tuple[pl.DataFrame, pl.DataFrame]]:
    """
...
def perform_cross_validation(
    ...
    for fold, (train_val_data, test_data) in enumerate(splits):
        ...
        # Train model
        trainer = ModelTrainer(model=model, loss_fn=loss_fn, optimizer=optimizer, device=device)
