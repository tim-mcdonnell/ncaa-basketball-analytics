...
class ModelEnsemble(BaseModel):
    ...
    def __init__(
        ...
        if config and hasattr(config, "hyperparameters"):
            ...
            self.config.hyperparameters = {"ensemble_method": ensemble_method, "weights": weights}
        ...
        if weights is None:
            ...
            if len(weights) != self.n_models:
                raise ValueError(
                    f"Number of weights ({len(weights)}) does not match number of models ({self.n_models})"
                )
    ...
    def predict(self, data: Union[DataLoader, torch.Tensor]) -> torch.Tensor:
        """
        Make predictions using the ensemble.

        Args:
            data: Either a DataLoader containing batches of data or a tensor of inputs
        ...
        if self.aggregation_method == "average":
            ...
            weights = self.weights.view(-1, 1, 1).to(stacked_preds.device)
            return torch.sum(stacked_preds * weights, dim=0)
        elif self.aggregation_method == "max":
            values, _ = torch.max(stacked_preds, dim=0)
            return values
    ...
    def load(cls, filepath: str) -> "ModelEnsemble":
        ...
        # Create ensemble
        ensemble = cls(
            models=models, config=config, weights=weights, aggregation_method=aggregation_method
        )
