...
def create_feature_vector(
    game_data: Union[Dict[str, Any], pl.DataFrame],
...
class GamePredictor:
    ...
    def __init__(
        ...
        if feature_columns is None:
            if hasattr(model, 'config') and hasattr(model.config, 'features'):
                ...
                raise ValueError("feature_columns must be provided if model does not have config.features")
        else:
    ...
    def predict_games(
        ...
        result_df = games_data.with_columns(
            pl.Series(pred_col_name, predictions_np)
        )
    ...
    def format_predictions(
        ...
        if predictions_df is not None:
            ...
            result_df = games_df.with_columns(
                pl.Series(pred_col, pred_values)
            )
        else:
            raise ValueError("Either predictions_df or both games_df and predictions must be provided")
        ...
        if self.prediction_type == "point_spread":
            result_df = result_df.with_columns(
                pl.abs(pl.col(pred_col)).alias("predicted_margin")
            )
